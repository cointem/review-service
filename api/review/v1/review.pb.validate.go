...
func (m *CreateReviewRequest) validate(all bool) error {
	...
	if val := m.GetServiceScore(); val <= 0 || val >= 6 {
		err := CreateReviewRequestValidationError{
			field:  "ServiceScore",
			reason: "value must be inside range (0, 6)",
		}
	...
	if val := m.GetExpressScore(); val <= 0 || val >= 6 {
		err := CreateReviewRequestValidationError{
			field:  "ExpressScore",
			reason: "value must be inside range (0, 6)",
		}
	...
	if l := utf8.RuneCountInString(m.GetContent()); l < 8 || l > 1000 {
		err := CreateReviewRequestValidationError{
			field:  "Content",
			reason: "value length must be between 8 and 1000 runes, inclusive",
		}
		if !all {
			return err
		}
...
func (m *AppealReviewRequest) validate(all bool) error {
	...
	if l := utf8.RuneCountInString(m.GetReason()); l < 8 || l > 1000 {
		err := AppealReviewRequestValidationError{
			field:  "Reason",
			reason: "value length must be between 8 and 1000 runes, inclusive",
		}
...
func (m *AuditReviewRequest) validate(all bool) error {
	...
	if l := utf8.RuneCountInString(m.GetOpReason()); l < 8 || l > 1000 {
		err := AuditReviewRequestValidationError{
			field:  "OpReason",
			reason: "value length must be between 8 and 1000 runes, inclusive",
		}
	...
	if l := utf8.RuneCountInString(m.GetOpUser()); l < 8 || l > 1000 {
		err := AuditReviewRequestValidationError{
			field:  "OpUser",
			reason: "value length must be between 8 and 1000 runes, inclusive",
		}
